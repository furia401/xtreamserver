...
if (isset($_GET['acao']) && $_GET['acao'] === 'lista-filmes') {
    $conexao = conectar_bd();

    $query_streams = "SELECT id, name FROM streams WHERE stream_type = 'movie' LIMIT 1000";
    $stmt_streams = $conexao->prepare($query_streams);
    $stmt_streams->execute();
    $resultado_streams = $stmt_streams->fetchAll(PDO::FETCH_ASSOC);

    echo json_encode($resultado_streams, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    exit;
}

if (isset($_GET['acao']) && $_GET['acao'] === 'filmes-salvar-info') {

    $dados = json_decode(file_get_contents('php://input'), true);

    $quantidade = is_array($dados) ? count($dados) : 0;
